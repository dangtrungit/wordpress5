...
class ZendvnHtml
{
   ...
   // ======================= DISPLAY PHAN TU LABEL===========================
   public function labelTag($val = '', $attr = array(), $options = null)
   {
      $valfor = preg_replace('/\s+/', '', strtolower($val));
      $strAttr = '';
      if(count($attr)>0){
      ...
      return '<lable for="' .$valfor. '" ' .$strAttr. '>' . $val . ':</label>';
   }
